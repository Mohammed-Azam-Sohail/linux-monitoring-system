...
*/5 * * * * bash -c 'source ${SCRIPT_DIR}/config.cfg; CPU=\$(top -bn1 | awk \"/Cpu/ {print \$2}\"); MEM=\$(free | awk \"/Mem:/ {print \$3/\$2 * 100.0}\"); DISK=\$(df / | awk "END{print \$5+0}"); LOAD=\$(awk \"{print \$1}\" /proc/loadavg); bash ${SCRIPT_DIR}/alerts.sh \"\$CPU\" \"\$MEM\" \"\$DISK\" \"\$LOAD\"' >> ${SCRIPT_DIR}/logs/alerts.log 2>&1
